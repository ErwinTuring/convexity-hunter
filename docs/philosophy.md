...
The approved target doctrine is [Core Convexity Research Doctrine v0.1](core-convexity-research-doctrine-v0.1.md).
It is a product-target correction approved independently of implementation
...
underpricing claim, does not issue a trade recommendation, and does not
monitor or execute a position.

## Events create hypotheses; market data supports numerical evidence
...
does not require an Event Intelligence hypothesis. The approved Discovery,
Event, and Direct entries converge on one exact structure standard and the
same Core evidence rules.
...
Straddle at a time. Direct entry may supply one exact verified structure; the
approved Discovery and Event target enumerates all deterministic eligible
structures without a human selection gate, ATM/Delta inference, or ranking.
...
Finite maximum loss and nonlinear payoff are inputs to price-and-risk-impact
research, not by themselves evidence of underpricing. The Core consumes only
cash flows explicitly declared by the selected strategy and required by the
chosen claim. Theta decay, liquidity, IV repricing, and other stronger analyses
are claim-triggered rather than mandatory Core inputs. Any statement that
convexity is underpriced still requires an explicit lawful benchmark.

Underpricing is an optional benchmark claim, not a Core gate. Missing an
underpricing benchmark or long option history does not invalidate Core
geometry; VolEnv, Tail, percentile, and other enhancements are required only
when a stated claim consumes them.
...
The human user remains responsible for direct-entry intent, portfolio-level
risk, and every investment or trading decision. Discovery and Event target
enumeration is not a recommendation.
