...
7. Implement the Skill-led event-to-underlying accepted contract and audit
   boundary.
8. Implement real option-chain access, contract verification, maturity and
   strike/Delta policy, supported structure generation, and layered selection.
...
- exact strike or Delta-selection policy and whether a 10-delta far-tail mode
  is included;
- actual mature Skills, their gaps, adapters, and compositions;
- the final risk-budget record;
